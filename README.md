...
- **Wrappable Statement Rendering (explicit structure, no frontend guessing):**
  - **Backend emits structure:** `build_db.py` walks the Wolfram TraditionalForm box AST with `ast_to_flow()` and emits explicit prose/math/space token lines (stored per entity as `statement_tokens`): string boxes become wrappable prose tokens, box structures become atomic inline-math tokens, delimiter grouping (`(...)`/`{...}`, incl. nesting), dangling-script gluing, and math-vs-text bolding (`\mathbf`/`\boldsymbol` vs `\textbf`) are all decided structurally at this layer.
  - **Frontend only lays out tokens:** `MathRenderer` flow mode renders the token lines verbatim (prose as wrappable text, math as atomic inline MathJax, explicit wrappable separators) so statements wrap over multiple lines instead of overflowing as one long unbreakable formula.
  - **Tests:** `backend/test_flow.py` covers representative topology notation (prose/math distinction, paren/brace grouping, set-builder braces, sub/superscripts, `\mathbf`/`\boldsymbol`/`\textbf`/`\textit`, multi-line grids) plus an end-to-end Ascoli theorem integration test. Run with `python3 -m unittest backend.test_flow -v`.
  - Upright operator names (`\text{Hom}_{\textbf{Top}}`) and the double-struck reals `\mathbb{R}` (Wolfram PUA character U+F7B5) are converted textbook-style.
